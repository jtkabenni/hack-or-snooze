...
class StoryList {
  ...
  static async deleteStory(story) {
    ...
    currentUser.ownStories = currentUser.ownStories.filter(
      (ownStory) => story.storyId !== ownStory.storyId
    );
  }
}
...
class User {
  ...
  static async addStoryToFavorite(story) {
    const response = await axios({
  ...
  static async removeStoryFromFavorite(story) {
    const response = await axios({
      url: `${BASE_URL}/users/${currentUser.username}/favorites/${story.storyId}`,
      method: "DELETE",
      data: {
    ...
    currentUser.favorites = currentUser.favorites.filter(
      (favorite) => story.storyId !== favorite.storyId
    );
  }
}
